...
async def app_client(_app_settings: AppSettings) -> AsyncIterator[AsyncClient]:
    ...
    target_app_settings.DATABASE_URI = urllib.parse.urlunparse(  # type: ignore
        urllib.parse.urlparse(_app_settings.DATABASE_URI)._replace(path=f'/{temp_db_name}')
    )
    ...
    async with LifespanManager(prepare_app):
        ...
        try:
            ...
            db_conn \
                ...
                .execute(f'CREATE DATABASE "{temp_db_name}" WITH TEMPLATE "{test_db_name}"')

            app = create_app(target_app_settings)
            async with AsyncClient(app=app, base_url='http://test') as app_client, LifespanManager(app):
                yield app_client
        finally:
            ...
            db_conn \
                .execution_options(isolation_level='AUTOCOMMIT') \
                .execute(f'DROP DATABASE IF EXISTS "{temp_db_name}"')
