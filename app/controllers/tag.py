...
def tag_post_api(
    ...
    is_tag_exists: bool = db_session \
        .scalar(
            sql_exp
            .exists()
            .where(m.Tag.name == q.name)
            .select()
        )
...
class _TagSearchRequest(BaseModel):
    filter_expr: Optional[Dict[str, Any]] = Field(description=_TagSearchRequestFilterExpr.description)
...
def tag_search_api(
    ...
    tags = tags_query \
        .order_by(
            sort_by_order_exp(sort_by_col)  # type: ignore
        ) \
        .slice(q.offset, q.offset + q.count) \
        .all()

    return [
        _TagSearchResponse.from_orm(tag)
        for tag in tags
    ]
